#!/usr/bin/env bash
# server-setup.sh
# Self-installing wrapper script.  Upload this file to GitHub; users who download it
# should run:
...
TMPFILE="$(mktemp /tmp/server-setup.XXXXXX)"
trap 'rm -f "$TMPFILE"' EXIT

cat > "$TMPFILE" <<'SERVER_EOF'
#!/bin/bash
...
# move into place using install (will use sudo if required)
INSTALL_TARGET="/usr/local/bin/setup-server.sh"
if ! install -m 0755 "$TMPFILE" "$INSTALL_TARGET" 2>/dev/null; then
  echo "Installation to /usr/local/bin requires root privileges. Asking sudo..."
  sudo install -m 0755 "$TMPFILE" "$INSTALL_TARGET"
fi

chmod +x "$INSTALL_TARGET" || true
rm -f "$TMPFILE" || true

echo "Installed $INSTALL_TARGET (executable)"

echo
cat <<'USAGE'
Now you can run the server installer as root:
  sudo /usr/local/bin/setup-server.sh <server-ip> <fqdn> <domain>
Examples:
  sudo /usr/local/bin/setup-server.sh 192.168.29.206 server.cst.com cst.com
  sudo /usr/local/bin/add-client.sh client99 192.168.29.199 cst.com
  sudo /usr/local/bin/admin-block-client.sh block 192.168.29.199
  sudo /usr/local/bin/admin-block-client.sh unblock 192.168.29.199
USAGE
